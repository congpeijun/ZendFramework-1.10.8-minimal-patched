 ...
 * @version    $Id: Imap.php,v 1.1 2010-10-25 15:20:48 h.wang Exp $
 */
                             ...
                             implements Zend_Mail_Storage_Folder_Interface, Zend_Mail_Storage_Writable_Interface
{
    ...
    {
        try {
        	$number = $this->UniqueId2Number($id); // Use the LinkORB method.
        	return $number;
    ...
    public function uidsearch(array $params)
    {
    	
        $response = $this->_protocol->requestAndResponse('UID SEARCH', $params);
        if (!$response) {
        	return $response;
        }
    ...
    public function uidsort(array $params, $sort="ARRIVAL")
    {
		$response = $this->_protocol->requestAndResponse("UID SORT (" . $sort . ") UTF-8", $params);
        if (!$response) {
        	//echo "NO RESPONSE ON UIDSORT";
        	//return $response;
        	return false;
        }
    ...
    public function capability () {
    	$cap = $this->_protocol->capability();
    	if (!$cap) {
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new Zend_Mail_Storage_Exception('Cannot fetch capability.');
        }
        return $cap;
    }
     ...
     * @throws Zend_Mail_Storage_Exception
     */
    public function UniqueId2Number($uid)
    {
        $response = $this->_protocol->requestAndResponse("SEARCH UID ".$uid);
        if ($response[0][0]=="SEARCH") {
        	if ($response[0][1]) return $response[0][1];
        }
        /*
        foreach ($response as $ids) {
        	if ($ids[0] == 'SEARCH') {
